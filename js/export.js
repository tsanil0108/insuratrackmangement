    ...
    <div style="background:linear-gradient(135deg,var(--accent-soft),var(--purple-soft));
      ...
      <div style="flex:1;min-width:200px;">
        ...
        <div style="font-size:13px;color:var(--text-secondary);">
          Send filtered policies directly to your email
        </div>
      </div>
...
async function loadDropdownData() {
  try {
    ...
    // Load companies
    const companiesRes = await fetch(api.buildUrl('v1/companies'), {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (companiesRes.ok) {
      companiesList = await companiesRes.json();
    }
    
    // Load insurance types
    const typesRes = await fetch(api.buildUrl('v1/insurance-types'), {
      headers: { 'Authorization': `Bearer ${token}` }
    });
    if (typesRes.ok) {
      insuranceTypesList = await typesRes.json();
    }
  } catch (error) {
...
function attachFilterListeners() {
  const filters = ['export-start-date', 'export-end-date', 'export-status', 
...
function clearExportFilters() {
  ...
  document.getElementById('export-payment-status').value = '';
  document.getElementById('export-company').value = '';
  document.getElementById('export-type').value = '';
  
  exportFilters = {
    startDate: '', endDate: '', status: '', paymentStatus: '', companyId: '', insuranceTypeId: ''
  };
...
function updateFilterCount() {
  ...
  if (badge) {
    badge.textContent = count > 0 ? `${count} filter${count > 1 ? 's' : ''} active` : '';
  }
}
...
async function exportFilteredPoliciesCSV() {
  try {
    ...
    await api.downloadCSV(`/export/policies/csv${queryString}`, `${filename}.csv`);
    window.hideSpinner();
...
async function exportFilteredPoliciesExcel() {
  try {
    ...
    const queryString = getFilterQueryString();
    const filename = getExportFilename(`policies_filtered`);
    await api.download(`/export/policies/excel${queryString}`, `${filename}.xlsx`);
    window.hideSpinner();
    window.showToast('Policies exported successfully!', 'success');
  } catch (error) { 
    window.hideSpinner();
...
async function exportFilteredPoliciesPDF() {
  try {
    ...
    const queryString = getFilterQueryString();
    const filename = getExportFilename(`policies_filtered`);
    await api.download(`/export/policies/pdf${queryString}`, `${filename}.html`);
    window.hideSpinner();
    window.showToast('Policies exported successfully!', 'success');
  } catch (error) { 
    window.hideSpinner();
...
async function emailPoliciesExport() {
  ...
  try {
    ...
    const token = localStorage.getItem('insura_token');
    const queryString = getFilterQueryString();
    const url = api.buildUrl(`/export/policies/email?email=${encodeURIComponent(email)}${queryString}`);
    ...
    if (response.ok) {
      window.showToast(`✅ Policies export sent to ${email}`, 'success');
    } else {
      const err = await response.json().catch(() => ({}));
      window.showToast(err.message || 'Email failed. Try downloading instead.', 'warning');
    }
  } catch (error) {
    window.hideSpinner();
...
console.log('Export module loaded with filters (Policies only)');